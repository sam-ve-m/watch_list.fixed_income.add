...
async def save_products(request: Request = request) -> Response:
    ...
    try:
        ...
        watch_list_products = WatchListProducts(**raw_params)
        unique_id = jwt_content["decoded_jwt"]["user"]["unique_id"]
        result = await WatchListService.register_products(
            watch_list_products=watch_list_products, unique_id=unique_id
        )
